 ...
 * ============================================================ */
static idpool_region *zone_registry[16][MAX_REGIONS] CACHE_ALIGN;

static inline idpool_region *registry_load(int zone, uint32_t idx) {
    if (idx >= MAX_REGIONS) return NULL;
    return (idpool_region *)a_load64((uint64_t *)&zone_registry[zone][idx]);
}

static inline void registry_publish(int zone, uint32_t idx, idpool_region *r) {
...
static idpool_region *create_region(idpool_zone *z) {
    ...
    for (;;) {
        ...
        if (a_cas32(&z->region_count, &exp, rc + 1)) {
            ...
            idpool_region *r = (idpool_region *)a_alloc(sizeof(idpool_region));
            if (!r) abort();
            memset(r, 0, sizeof(*r));
            r->base = base;
            r->cap = cap;
 ...
 * 加载 Region (通过 slot)
 * ============================================================ */
static idpool_region *load_region(idpool_zone *z, uint32_t idx, uint64_t *ver_out) {
...
fun_idpool_t fun_idpool_create(int numa_nodes) {
    ...
    for (int n = 0; n < actual; n++) {
        ...
        idpool_region *r0 = (idpool_region *)a_alloc(sizeof(idpool_region));
        if (!r0) abort();
